...
def e_k(data):
    d = data
    dA = np.exp(-apply(d[:, 0:5], lambda x: get_k(times[3:8], x), 0))
    dB = np.exp(-apply(d[:, 5:10], lambda x: get_k(times[3:8], x), 0))
    return [dA, dB]


boot_k = get_bootstrap(normData1, e_k, n)
...
def C(data):
    d = data
    dA = np.exp(apply(d[:, 0:5], lambda x: get_C(times[3:8], x), 0))
    dB = np.exp(apply(d[:, 5:10], lambda x: get_C(times[3:8], x), 0))
    return [dA, dB]


boot_C = get_bootstrap(normData1, C, n)
